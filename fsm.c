...
void mixer_fsm() {
	...
	case MOVE_UP_MIXER:
		...
		break;
	case PREHEAT_OVEN:
		if(temperature >= BAKING_TEMP){		//if oven has baking temperature
			pwm_value_oven = 0;				//stop heating
			mixer_current_state = WAIT_OUT_MIXER;
		}
		else {
			if (oven_current_state != BAKE_OVEN)	//if there is no cake baking in the oven
				pwm_value_oven = 10000;		//heat up the oven
		}
		break;
	case WAIT_OUT_MIXER:
		if (oven_current_state == WAIT_IN_OVEN) { // if the oven avaliable
			set(MFBM); // start moving the band under the mixer
...
void oven_fsm() {
	...
	case WAIT_IN_OVEN:
		if (mixer_current_state == MOVE_OUT_MIXER) {// if the mixer is done with process we can get the box in oven
			set(MFBO);
			oven_current_state = MOVE_IN_OVEN;
		}
		break;
	case MOVE_IN_OVEN:
		if (!get24(SIO)) {//if the box is in right place in oven we stop the band
			reset(MFBO);
			oven_time = 0;
		...
		break;
	case BAKE_OVEN:
		oven_time++; //for every sycle we increase the baking time
		if((BAKING_TEMP - temperature) < 5){ //if temperature inside the oven is between 30 and 35 we use our P controller to set pwm value
			pwm_value_oven = 2000 * (int)(BAKING_TEMP - temperature);
			if(pwm_value_oven >= 10000){//we set a cover system to stop the oven in a permanent pwm value before system gets too hot
		...
		if (oven_time == BAKING_TIME) { //when the 30 sec finishes we stop the baking and send our box out of the oven
			pwm_value_oven = 0;
			set(MFBO);
		...
		break;
	case MOVE_OUT_OVEN:
		if (get24(SLA)) { //after box gets out of the system we stop our band.
			reset(MFBO);
